# Aurora Coffee Backend Service

Basic online store backend service made with Node.JS for the CS308 online store project.

## Commands

- Go to the frontend's directory
- Complete the commands [here](https://github.com/dsicim/AuroraCoffee/tree/main/Frontend)
- Go to the backend's directory
- Configure the backend service by editing the [config.json](https://github.com/dsicim/AuroraCoffee/blob/main/Backend/config.json.example) file and remove `.example` from the name.
- `npm i`
- `node .`

## Configuration File