...
async function handleAPI(config, method, endpoint, query, body, headers, currentUser) {
    if (endpoint[0] === "carddetails") {
        if (method === "POST") {
            ...
            const response = await IyzipayAPI(config, "POST", "payment/bin/check", {}, {locale:"tr",binNumber: body.bin});
            if (response) {
                if (response.status === "success") {
                    return { s: 200, j: true, d: { response: response } };
                }
                else {
                    return { s: 400, j: true, d: { e: "Failed to fetch card details from payment provider", response: response } };
                }
            }
            else {
                return { s: 500, j: true, d: { e: "An unknown error occurred while communicating with the payment provider", response: response } };
            }
        }
