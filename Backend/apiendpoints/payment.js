...
async function createOrder(config, currentUser, cart, basket, subtotal, shippingAddress, billingAddress, card, cardDetails, installment = 1, currency = "TRY", exchangeRate = 1) {
    ...
    const details = {
        ...
        card: {
            ...
            last4dig: cardDetails.last4dig || String(card.number).substring(String(card.number).length - 4)
        },
        installment: ins,
    ...
    const payload = {
        ...
        buyer: {
            ...
            identityNumber: currentUser.tax_id, // We are not collecting this information, but it's required by the payment provider, so we will just send a dummy value
...
module.exports = { handleAPI, IyzipayAPI };
