...
async function handleAPI(config, method, endpoint, query, body, headers, currentUser) {
    ...
    if (endpoint[0] === "me") {
        if (method === "GET") return { s: 200, j: true, d: { user: currentUser } };
            ...
            if (!body || !body.exists || body.err || !body.json || !body.data || ((!body.data.name || !body.data.privacy) && body.data.emailblock === undefined && body.data.taxId === undefined && body.data.tax_id === undefined)) return { s: 400, j: true, d: { e: "Invalid request body" } };
            ...
            const taxId = body.data.taxId === undefined ? body.data.tax_id : body.data.taxId;
            const nextTaxId = taxId === undefined ? currentUser.tax_id : String(taxId || "").trim();
            if (nextTaxId && nextTaxId.length > 50) return { s: 400, j: true, d: { e: "Tax ID must be 50 characters or fewer" } };
            return await sql.editUser(currentUser.id, body.data.name, body.data.privacy, body.data.emailblock === undefined ? currentUser.emailblock : body.data.emailblock, nextTaxId || null).then(res => {
                if (res.success) {
